...
class _RegistrationScreenState extends State<RegistrationScreen> {
  ...
  void initState() {
    ...
    controllerName.addListener(() {
      final isButtonActive = (controllerName.text.length>1)&&controllerName.text.isNotEmpty;
      setState(() {
        isButtonActiveName = isButtonActive;
      });
    });
    controllerPasswordConfirm.addListener(() {
      final isButtonActive = (controllerPasswordConfirm.text == controllerPassword.text)&&controllerPasswordConfirm.text.isNotEmpty;
      setState(() {
        ...
        builder: (context, state){
          ...
          if(state is RegLoading){
            ...
            WidgetsBinding.instance?.addPostFrameCallback((timeStamp) {
              ...
              BlocProvider.of<RegCubit>(context).initialState();
            });
          }
    ...
    return SafeArea(
      child: Column(
        ...
        children: [
          Column(
            children: [
              const HeaderWidget(title: "Регистрация в 7Food", subtitle: 'Введите номер телефона, чтобы войти в приложение через СМС-код'),
          ...
          Column(
            children: [
              ...
              Padding(
                ...
                child: BlueButton(callback: (isButtonActiveName&&isButtonActiveNumber&&isButtonActivePassword&&isButtonActiveConfirm)?(){
                  BlocProvider.of<RegCubit>(context).registering(_name!, "+7 $_phoneNumber", _password!, _password!);
                  controllerName.clear();
                  controller.clear();
                  controllerPassword.clear();
                  controllerPasswordConfirm.clear();
                }:null, title: "ЗАРЕГИСТРИТОВАТЬСЯ"),
              ),
